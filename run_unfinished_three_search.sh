#!/usr/bin/env bash
# scripts/run_unfinished_three_search.sh
#
# 重新训练/补跑上次未完成的 3 个标的：
#   002311.SZ 海大集团：sector 改为 农产品加工，优先 external feed,hog
#   002714.SZ 牧原股份：重新尝试 external hog,muyuan_hk，muyuan_hk 配置已修为 02714
#   600176.SH 中国巨石：sector 改为 建筑材料
#
# 逻辑：
#   优先 external；
#   external 失败/超时 → fallback 到带 sector 的 baseline；
#   sector baseline 失败/超时 → fallback 到 fundamental-only baseline；
#   任一标的失败不会阻塞后续标的。
#
# 用法：
#   PYTHON=python3 ./run_unfinished_three_search.sh
#   JOB_TIMEOUT=6h PYTHON=python3 ./run_unfinished_three_search.sh

set -uo pipefail

PYTHON="${PYTHON:-python3}"
END_DATE="${END_DATE:-$(date +%F)}"
JOB_TIMEOUT="${JOB_TIMEOUT:-8h}"
LOG_DIR="${LOG_DIR:-saved_data/model_search_queue_logs/unfinished_three_$(date +%Y%m%d_%H%M%S)}"

mkdir -p "$LOG_DIR"

COMMON_ARGS=(
  --start-date 2018-01-01
  --end-date "$END_DATE"
  --search-targets hit50,hit80,close_profit
  --entry-policies vwap_low,all_days
  --quantiles 0.5,0.6,0.7,0.8
  --train-rows 756
  --valid-rows 126
  --test-rows 63
  --min-valid-trades 8
  --min-train-entries 80
  --resume
  --excel
)

MODELS_ALL=(
  --models xgb_d2_200_lr003_mcw5,xgb_d3_400_lr003_mcw3,xgb_d3_600_lr002_mcw3,xgb_d4_500_lr002_mcw5,lgbm_leaves7_400,lgbm_leaves15_700,extra_trees_600_d3,random_forest_600_d4
)

GROUPS_WITH_SECTOR=(
  --groups reversal_fundamental_regime,reversal_fundamental_regime_sector,all_no_ak
)

GROUPS_FUND_ONLY=(
  --groups reversal_fundamental_regime,all_no_ak
)

SUMMARY_FILE="$LOG_DIR/queue_summary.csv"
echo "symbol,sector,mode,external,status,returncode,start_time,end_time,log_file" > "$SUMMARY_FILE"

run_pipeline() {
  local symbol="$1"
  local sector="$2"
  local mode="$3"
  local external="${4:-}"

  local safe_symbol="${symbol//./_}"
  local log_file="$LOG_DIR/${safe_symbol}_${mode}.log"
  local start_time
  local end_time
  local rc

  start_time="$(date '+%F %T')"

  echo "============================================================" | tee -a "$log_file"
  echo "[START] ${start_time} symbol=${symbol}, sector=${sector}, mode=${mode}, external=${external:-none}" | tee -a "$log_file"
  echo "============================================================" | tee -a "$log_file"

  if [[ "$mode" == "fundamental_only" ]]; then
    timeout --foreground "$JOB_TIMEOUT" "$PYTHON" pipelines/run_nextday_pipeline.py \
      --symbol "$symbol" \
      --feature-pipeline fundamental \
      "${GROUPS_FUND_ONLY[@]}" \
      "${MODELS_ALL[@]}" \
      "${COMMON_ARGS[@]}" 2>&1 | tee -a "$log_file"
    rc=${PIPESTATUS[0]}
  elif [[ -n "$external" ]]; then
    timeout --foreground "$JOB_TIMEOUT" "$PYTHON" pipelines/run_nextday_pipeline.py \
      --symbol "$symbol" \
      --sector-symbol "$sector" \
      --feature-pipeline fundamental,sector \
      --external "$external" \
      "${GROUPS_WITH_SECTOR[@]}" \
      "${MODELS_ALL[@]}" \
      "${COMMON_ARGS[@]}" 2>&1 | tee -a "$log_file"
    rc=${PIPESTATUS[0]}
  else
    timeout --foreground "$JOB_TIMEOUT" "$PYTHON" pipelines/run_nextday_pipeline.py \
      --symbol "$symbol" \
      --sector-symbol "$sector" \
      --feature-pipeline fundamental,sector \
      "${GROUPS_WITH_SECTOR[@]}" \
      "${MODELS_ALL[@]}" \
      "${COMMON_ARGS[@]}" 2>&1 | tee -a "$log_file"
    rc=${PIPESTATUS[0]}
  fi

  end_time="$(date '+%F %T')"

  if [[ "$rc" -eq 0 ]]; then
    echo "[DONE] ${end_time} symbol=${symbol}, mode=${mode}, status=ok" | tee -a "$log_file"
    echo "${symbol},${sector},${mode},${external:-},ok,${rc},${start_time},${end_time},${log_file}" >> "$SUMMARY_FILE"
    return 0
  elif [[ "$rc" -eq 124 ]]; then
    echo "[TIMEOUT] ${end_time} symbol=${symbol}, mode=${mode}, timeout=${JOB_TIMEOUT}" | tee -a "$log_file"
    echo "${symbol},${sector},${mode},${external:-},timeout,${rc},${start_time},${end_time},${log_file}" >> "$SUMMARY_FILE"
    return "$rc"
  else
    echo "[FAIL] ${end_time} symbol=${symbol}, mode=${mode}, returncode=${rc}" | tee -a "$log_file"
    echo "${symbol},${sector},${mode},${external:-},failed,${rc},${start_time},${end_time},${log_file}" >> "$SUMMARY_FILE"
    return "$rc"
  fi
}

run_one() {
  local symbol="$1"
  local sector="$2"
  local external="${3:-}"

  echo
  echo "############################################################"
  echo "# QUEUE symbol=${symbol}, sector=${sector}, external=${external:-none}"
  echo "############################################################"

  if [[ -n "$external" ]]; then
    if run_pipeline "$symbol" "$sector" "external" "$external"; then
      echo "[QUEUE] ${symbol}: external succeeded."
      return 0
    else
      echo "[QUEUE] ${symbol}: external failed/timeout, fallback to baseline with sector."
    fi
  fi

  if run_pipeline "$symbol" "$sector" "baseline" ""; then
    echo "[QUEUE] ${symbol}: baseline succeeded."
    return 0
  else
    echo "[QUEUE] ${symbol}: baseline failed/timeout, fallback to fundamental-only."
  fi

  if run_pipeline "$symbol" "$sector" "fundamental_only" ""; then
    echo "[QUEUE] ${symbol}: fundamental-only fallback succeeded."
    return 0
  else
    echo "[QUEUE] ${symbol}: all modes failed. Continue next symbol."
    return 1
  fi
}

FAILED=0

run_one 002311.SZ 农产品加工 "feed,hog" || FAILED=$((FAILED + 1))
run_one 002714.SZ 养殖业 "hog,muyuan_hk" || FAILED=$((FAILED + 1))
run_one 600176.SH 建筑材料 || FAILED=$((FAILED + 1))

echo
echo "============================================================"
echo "[ALL DONE] $(date '+%F %T')"
echo "[FAILED SYMBOLS] ${FAILED}"
echo "[SUMMARY] ${SUMMARY_FILE}"
echo "============================================================"

exit 0
