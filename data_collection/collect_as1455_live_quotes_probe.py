#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Probe live A-share quote collection stability for the AS1455 NN pipeline.

Scope of this first version
---------------------------
This standalone probe script tests whether the live quote source can
stably return the full AS1455 universe, typically 1000 symbols, over a small
number of rounds. It does not replace the old monitor code, does not build
Ch12 features, does not load a neural network, and does not produce orders.
...
DEFAULT_OUT_ROOT = PROJECT_DIR / "saved_data" / "ashare_ml4t" / "live_as1455_probe"
...
SNAPSHOT_COLUMNS = [
    "probe_round", "symbol", "code", "exchange", "name", "trade_date", "source_trade_date",
...
def run_probe(args: argparse.Namespace) -> None:
    """Run a short multi-round full-universe pull probe."""
    trade_date = args.trade_date or today_yyyymmdd()
    universe = load_universe(args.universe, args.max_symbols)
    day_dir = output_day_dir(args.out_root, trade_date)
    day_dir.mkdir(parents=True, exist_ok=True)
    ...
    for round_no in range(1, int(args.rounds) + 1):
        ...
        for batch_no, batch in enumerate(chunks(symbols, args.batch_size), start=1):
            ...
            for symbol in batch:
                raw = got.get(symbol)
                if raw is None:
                    row = error_snapshot(symbol, collected_at, trade_date, "missing_sina_payload", args.include_raw)
                else:
                    row = asdict(parse_sina_payload(symbol, raw, collected_at, args.include_raw))
                row["probe_round"] = round_no
                rows.append(row)
            if args.batch_sleep_seconds > 0 and batch_no < math.ceil(len(symbols) / args.batch_size):
                time.sleep(args.batch_sleep_seconds)
        ...
        elapsed = time.monotonic() - round_start_mono
        got_payload = int((df["source_status"].fillna("") != "missing_sina_payload").sum()) if len(df) else 0
        core_complete = int(df["core_complete"].fillna(False).astype(bool).sum()) if len(df) else 0
        source_date_counts = df["source_trade_date"].fillna("missing").value_counts().to_dict() if len(df) else {}
        source_time_missing = int(df["source_trade_time"].isna().sum()) if len(df) and "source_trade_time" in df.columns else 0
        source_status_counts = df["source_status"].fillna("ok_or_blank").value_counts().to_dict() if len(df) else {}
        volume_unit_counts = df["volume_unit_inferred"].fillna("missing").value_counts().to_dict() if len(df) else {}

        summary = {
            "probe_round": round_no,
            "round_started_at": round_start_wall.isoformat(timespec="seconds"),
            "elapsed_seconds": round(elapsed, 3),
            "expected_symbols": int(len(universe)),
            "snapshot_rows": int(len(df)),
            "symbols_returned_payload": got_payload,
            "payload_rate": got_payload / len(universe) if len(universe) else None,
            "core_complete_rows": core_complete,
            "core_complete_rate": core_complete / len(universe) if len(universe) else None,
            "requested_batches": requested_batches,
            "ok_batches": ok_batches,
            "error_batches": int(len(errors)),
            "source_time_missing_rows": source_time_missing,
            "source_trade_date_counts": json.dumps({str(k): int(v) for k, v in source_date_counts.items()}, ensure_ascii=False),
            "source_status_counts": json.dumps({str(k): int(v) for k, v in source_status_counts.items()}, ensure_ascii=False),
            "volume_unit_counts": json.dumps({str(k): int(v) for k, v in volume_unit_counts.items()}, ensure_ascii=False),
        }
        round_summaries.append(summary)
        print(json.dumps(summary, ensure_ascii=False, indent=2))

        if round_no < int(args.rounds):
            time.sleep(max(1, int(args.interval_seconds)))

    round_df = pd.DataFrame(round_summaries)
    write_csv(day_dir / "probe_round_summary.csv", round_df)
    ...
    final_summary = {
        ...
        "min_payload_rate": float(round_df["payload_rate"].min()) if len(round_df) else None,
        "min_core_complete_rate": float(round_df["core_complete_rate"].min()) if len(round_df) else None,
        "max_elapsed_seconds": float(round_df["elapsed_seconds"].max()) if len(round_df) else None,
        "total_error_batches": int(sum(x.get("error_batches", 0) for x in round_summaries)),
        "probe_passed": bool(
            len(round_df) > 0
            and float(round_df["payload_rate"].min()) >= float(args.min_payload_rate)
            and float(round_df["core_complete_rate"].min()) >= float(args.min_core_complete_rate)
        ),
        "min_payload_rate_required": float(args.min_payload_rate),
        "min_core_complete_rate_required": float(args.min_core_complete_rate),
        "out_dir": str(day_dir),
        "notes": [
            "This is a source-pull stability probe, not a final 14:55 feature panel.",
            "Use probe_round_summary.csv to judge whether 1000 symbols can be pulled reliably.",
        ],
    }
    (day_dir / "probe_summary.json").write_text(json.dumps(final_summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    sp = sub.add_parser("probe", help="Short multi-round 1000-symbol source stability probe")
    add_common(sp)
    sp.add_argument("--rounds", type=int, default=3, help="Number of pull rounds; default 3")
    sp.add_argument("--interval-seconds", type=int, default=10, help="Seconds between probe rounds; default 10")
    sp.add_argument("--min-payload-rate", type=float, default=0.99, help="Minimum payload return rate per round")
    sp.add_argument("--min-core-complete-rate", type=float, default=0.98, help="Minimum complete core-field rate per round")
    sp.add_argument("--finalize", action="store_true", help="Also build latest/panel/report after probe")
