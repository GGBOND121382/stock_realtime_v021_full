...
def signal_specs_for_top_n(top_n: int) -> list[str]:
    """Return grid ``--signal-spec`` values valid for prediction columns.

    With five checkpoints this preserves the historical seven signals. With one
    checkpoint it runs only ``model_0``. With three checkpoints it emits only
    ``ensemble_first3_mean`` because ``ensemble_all3_mean`` would be identical.
    """
    if top_n < 1:
    ...
    if top_n >= 2 and top_n != 3:
        columns = ",".join(str(index) for index in range(top_n))
        name = (
            "ensemble_all5_mean"
            if top_n == 5
            else f"ensemble_all{top_n}_mean"
        )
