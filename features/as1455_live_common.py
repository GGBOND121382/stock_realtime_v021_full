...
def compute_ch12_features(prices: pd.DataFrame, universe_meta: pd.DataFrame, include_forward_labels: bool = False) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Keep MultiIndex order as (date, symbol).  The old swaplevel() changed it to
    ...
    price_codes = pd.Series(prices.index.get_level_values("symbol").map(symbol_code), index=prices.index)
