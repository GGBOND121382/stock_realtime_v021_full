...
def resolve_repo_path(raw: Any, stock_code: str = "") -> Optional[Path]:
    ...
    # Conservative fallback: search only stock-specific pipeline dirs.  Many
    # artifacts use the same basename such as training_samples_with_sector.csv;
    # a global saved_data search can silently bind one stock to another stock's
    # sample file.
    code = normalize_stock_code(stock_code).split(".", 1)[0] if stock_code else ""
    name = Path(text).name
    roots = []
    if code:
        saved_data = PROJECT_DIR / "saved_data"
        roots.extend(sorted(saved_data.glob(f"{code}_pipeline_out*")))
        roots.extend(sorted(saved_data.glob(f"{code}_base_out*")))
    if name:
        for root in roots:
            if not root.exists():
                continue
...
def load_artifact_states(
    ...
    for meta_path in discover_artifacts(models_dir, watchlist, model_policy):
        ...
        raw_intraday = meta.get("intraday_bars")
        samples_path = resolve_repo_path(raw_samples, stock)
        intraday_path = resolve_repo_path(raw_intraday, stock)
        ...
        # Keep historical scoring on the exact same feature/return semantics as
        # model_training/search_walk_forward_model_complexity.py::make_dataset.
        # Saved samples usually do not contain derived reversal/regime features;
        # those must be rebuilt before model scoring instead of being filled by
        # training medians.
        samples = add_reversal_features(samples, intraday_path)
        ...
        if not cols:
            ...
            continue
        absent_cols = [c for c in cols if c not in samples.columns]
        if absent_cols:
            missing_features.append({
                ...
                "intraday_bars": str(intraday_path) if intraday_path else str(raw_intraday),
                "missing_features": absent_cols,
            })
            continue
    ...
    if missing_features:
        print("\n[ERROR] Some saved model artifacts are missing required feature columns after rebuilding features.\n", file=sys.stderr)
        for x in missing_features[:50]:
            missing = x["missing_features"]
            print(f"- {x['stock_code']} | {x['artifact_name']}", file=sys.stderr)
            print(f"  metadata:      {x['metadata']}", file=sys.stderr)
            print(f"  samples:       {x['samples']}", file=sys.stderr)
            print(f"  intraday_bars: {x['intraday_bars']}", file=sys.stderr)
            print(f"  missing:       {missing[:30]} (total={len(missing)})", file=sys.stderr)
        if len(missing_features) > 50:
            print(f"... and {len(missing_features)-50} more", file=sys.stderr)
        raise SystemExit(2)
