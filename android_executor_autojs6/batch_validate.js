...
function loadJsonIfExists(path) {
  ...
  try { return JSON.parse(files.read(path)); } catch (e) {
    throw new Error("invalid JSON state file: " + path + "; " + String(e));
  }
}
...
function run() {
  ...
  var previousRaw = loadJsonIfExists(previousPath);
  var previous = previousRaw ? batch.restoreState(previousRaw, fingerprint, orders, totalNotional, CSV_NAME) : null;
  var previousMatches = !!(previousRaw && previous === previousRaw);
  var unresolved = previousMatches ? batch.unresolvedRows(previous) : [];
  ...
  var report = {
    ...
    previous_state_matches_csv: previousMatches,
