#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AS1455 in-process grid using the single v7 trading engine.

This runner keeps data loading in one process while delegating every portfolio
simulation to ``run_as1455_close_auction_backtest_v7_maxpos_grid.backtest``.
It intentionally does not maintain a second buy/sell/NAV implementation.

Current optimization boundary:

- predictions are loaded once per signal;
- the execution panel is built once;
- universe/ST/corporate-action/capacity inputs are loaded once;
- all grid configurations run in one process;
- daily ranking is still performed by the v7 engine for each configuration.

Any future shared-ranking optimization must be implemented inside the common v7
backtest core, not by copying the trading loop into this grid runner.
"""
...
from utils.as1455_backtest_io import build_trade_config, write_run  # noqa: E402

ENGINE_NAME = "inprocess_shared_data_v3"
...
legacy = load_module(
    "as1455_grid_legacy", HERE / "run_as1455_close_auction_grid_v1.py"
)
bt = load_module(
    "as1455_bt_v7", HERE / "run_as1455_close_auction_backtest_v7_maxpos_grid.py"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="AS1455 in-process grid using the single v7 trade engine"
    )
...
def load_shared_inputs(args: argparse.Namespace) -> tuple[
    ...
    for spec in args.signal_specs:
        ...
        all_symbols.update(predictions["symbol"].unique())
...
def engine_manifest(
    *,
    ...
    return {
        ...
        "ranking_note": (
            "Ranking remains inside the shared v7 backtest for each config. "
            "Future ranking-cache work must refactor the common v7 core."
        ),
