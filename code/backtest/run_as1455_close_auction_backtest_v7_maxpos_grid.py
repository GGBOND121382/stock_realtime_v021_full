...
def resolve_prediction_column(df: pd.DataFrame, requested: object) -> object:
    """Resolve a user-supplied column token against possibly integer HDF columns."""
    ...
    if re.fullmatch(r"[-+]?\d+", s):
        ...
        if i in df.columns:
            return i
    raise KeyError(f"prediction score column {requested!r} not found; columns={list(df.columns)}")


def load_prediction_params(path: Path | None, selected_cols: list[object]) -> dict[str, object]:
    """Best-effort metadata reader for best_params.csv-like files.

    The exact training artifact format is not guaranteed, so this function never
    fails the backtest. It only records a small preview and, when possible, rows
    corresponding to selected model column indices.
    """
    if path is None or not path.exists():
        return {"model_params_file": str(path) if path else None, "model_params_found": False}
    try:
        df = pd.read_csv(path)
    except Exception as exc:
        return {"model_params_file": str(path), "model_params_found": False, "model_params_error": str(exc)}
    out: dict[str, object] = {
        "model_params_file": str(path),
        "model_params_found": True,
        "model_params_rows": int(len(df)),
        "model_params_columns": [str(c) for c in df.columns],
    }
    # Common convention: prediction columns 0..4 correspond to top rows 0..4.
    ...
    for i in selected_indices:
        if 0 <= i < len(df):
            ...
            row = {str(k): json_default(v) for k, v in row.items()}
            row["prediction_col"] = i
            rows.append(row)
    out["selected_model_params"] = rows
    return out
...
def load_predictions(
    ...
    candidate_cols = [c for c in df.columns if c not in {"symbol", "date"}]
    ...
    requested_cols = signal_cols or []
    if requested_cols:
        selected_cols = [resolve_prediction_column(df, c) for c in requested_cols]
    elif score_col is not None:
        selected_cols = [resolve_prediction_column(df, score_col)]
    else:
        selected_cols = [numeric_candidates[0]]

    mode = signal_mode or ("mean" if len(selected_cols) > 1 else "single")
    mode = str(mode).lower()
    if mode not in {"single", "mean", "median"}:
        raise ValueError(f"unsupported signal_mode={signal_mode!r}; expected single/mean/median")
    if mode == "single" and len(selected_cols) != 1:
        raise ValueError("signal_mode=single requires exactly one --signal-cols/--score-col")

    score_frame = pd.concat([pd.to_numeric(df[c], errors="coerce") for c in selected_cols], axis=1)
    score_frame.columns = [str(c) for c in selected_cols]
    if mode == "single":
        score = score_frame.iloc[:, 0]
    elif mode == "mean":
        score = score_frame.mean(axis=1, skipna=True)
    else:
        score = score_frame.median(axis=1, skipna=True)
    ...
    selected_cols_str = [str(c) for c in selected_cols]
    sig_name = signal_name or (f"model_{selected_cols_str[0]}" if mode == "single" else f"{mode}_{'_'.join(selected_cols_str)}")
    meta: dict[str, object] = {
        "prediction_file": str(path),
        "prediction_file_sha256": prediction_file_sha256 or sha256_file(path),
        "prediction_hdf_key": source_key,
        "prediction_available_score_cols": [str(c) for c in numeric_candidates],
        "signal_name": str(sig_name),
        "signal_mode": mode,
        "signal_cols": ",".join(selected_cols_str),
        "signal_cols_list": selected_cols_str,
        "score_col": selected_cols_str[0] if mode == "single" else None,
        "n_prediction_rows_raw": raw_rows,
        "n_prediction_rows": int(len(out)),
        "n_prediction_symbols": int(out["symbol"].nunique()),
        "n_prediction_dates": int(out["date"].nunique()),
        "prediction_date_min": out["date"].min(),
        "prediction_date_max": out["date"].max(),
    }
    meta.update(load_prediction_params(model_params_file, selected_cols))
...
def main() -> None:
    ...
    parser.add_argument("--score-col", default=None, help="Backward-compatible single score column. Prefer --signal-cols for grid runs.")
    parser.add_argument("--signal-name", default=None, help="Signal label written to logs, e.g. model_0 or ensemble_all5_mean")
    parser.add_argument("--signal-cols", default=None, help="Comma-separated prediction columns used for the signal, e.g. 0 or 0,1,2")
    parser.add_argument("--signal-mode", choices=["single", "mean", "median"], default=None, help="How to combine --signal-cols. Default: single for one col, mean for multiple cols.")
    parser.add_argument("--prediction-file-sha256", default=None, help="Optional precomputed SHA256 to avoid hashing the same prediction file in every grid run")
    parser.add_argument("--model-family", default="ML4T Ch17 NN", help="Model family metadata written to summary/config")
    parser.add_argument("--model-run", default=None, help="Training run name metadata, e.g. ch17_as1455_train_20260622_cv7")
    parser.add_argument("--model-params-file", default=None, help="Optional best_params.csv-like file for selected prediction columns")
    ...
    parser.add_argument("--write-execution-panel", action="store_true", help="Also write execution_panel.csv.gz; ignored unless --output-mode full")
    parser.add_argument(
        "--output-mode",
        choices=["summary", "compact", "full"],
        default="compact",
        help=(
            "Per-run file retention. summary keeps only JSON summaries/config; "
            "compact also keeps small conclusion CSVs such as nav/drawdown/monthly/fee/turnover; "
            "full keeps orders/positions/rejections/round_trips and other audit CSVs."
        ),
    )
    ...
    signal_cols = parse_csv_tokens(args.signal_cols)
    preds, signal_meta = load_predictions(
        ...
        prediction_file_sha256=args.prediction_file_sha256,
        model_params_file=Path(args.model_params_file) if args.model_params_file else None,
    )
    ...
    if args.write_execution_panel and args.output_mode == "full":
        exec_panel.to_csv(out_dir / "execution_panel.csv.gz", index=False, compression="gzip")
    elif args.write_execution_panel and args.output_mode != "full":
        print("[WARN] --write-execution-panel is ignored unless --output-mode full")
    ...
    if args.output_mode == "full":
        output_map = output_map_full
    elif args.output_mode == "compact":
        output_map = output_map_compact
    else:
        output_map = {}
    ...
    model_meta = {
        ...
        "model_run": str(args.model_run) if args.model_run else None,
    }
    ...
    run_meta = {
        ...
        "suppressed_output_files": sorted(set(output_map_full.keys()) - set(output_map.keys())),
    }
    config_payload = dict(cfg.__dict__)
    config_payload.update(model_meta)
    config_payload.update(signal_meta)
    config_payload["output_mode"] = str(args.output_mode)
    with open(out_dir / "config.json", "w", encoding="utf-8") as f:
        json.dump(config_payload, f, ensure_ascii=False, indent=2, default=json_default)
