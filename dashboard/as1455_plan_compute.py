#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Start-date-aware 14:55 plan computation used only by background materialization.

This module contains the former on-demand dashboard replay logic.  The Streamlit
page no longer calls it directly; changing the tracking start date triggers a
background materialization job which calls ``compute_nine_strategy_day`` once
per saved live date and persists the results.
"""
...
def compute_nine_strategy_day(
    ...
    *,
    ...
    """Compute one day's nine-strategy plan for background materialization only."""
    ...
    if capacity_mode != "none":
        raise RuntimeError("plan materialization currently requires capacity_mode=none")
