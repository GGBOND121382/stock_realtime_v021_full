...
def discover_experiment_names(matrix_root: Path) -> list[str]:
    ...
    if expected_file.is_file():
        names = [line.strip() for line in expected_file.read_text(encoding="utf-8").splitlines() if line.strip()]
    elif matrix_root.is_dir():
        names = [item.name for item in matrix_root.iterdir() if item.is_dir() and EXPERIMENT_RE.fullmatch(item.name)]
    else:
        names = []
    valid: list[str] = []
    for name in names:
...
def _decorate_summary(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["display_name"] = [x.display_name if x else str(name) for x, name in zip(identities, summary["experiment"])]
    if "rebalance_every" not in summary.columns:
        summary["rebalance_every"] = [x.rebalance_every if x else None for x in identities]
    ...
    summary["signal_order"] = summary["signal"].map({"all5": 0, "first3": 1, "best": 2})
    return summary.sort_values(["rebalance_every", "signal_order"], kind="stable").drop(columns=["signal_order"]).reset_index(drop=True)
...
def load_matrix_summary(matrix_root: Path) -> pd.DataFrame:
    ...
    return _decorate_summary(summary)


def load_tracking_matrix_summary(matrix_root: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    manifest = read_json(matrix_root / TRACKING_MATRIX_MANIFEST, {}) or {}
    summary = _decorate_summary(read_csv(matrix_root / TRACKING_MATRIX_SUMMARY))
...
def find_forward_run_dir(experiment_root: Path, manifest: dict[str, Any]) -> Path | None:
    ...
    if direct and direct.is_dir():
        return direct
    strict_manifest_path = experiment_root / "strict_oos_forward" / "01_close_auction_grid" / "strict_oos_manifest.json"
...
def normalize_nav(nav: pd.DataFrame, baseline: float | None = None) -> pd.DataFrame:
    if nav.empty or not {"date", "nav"}.issubset(nav.columns):
        return pd.DataFrame()
    work = nav.copy()
    work["date"] = pd.to_datetime(work["date"], errors="coerce").dt.normalize()
    work["nav"] = pd.to_numeric(work["nav"], errors="coerce")
    work = work.dropna(subset=["date", "nav"]).sort_values("date").drop_duplicates("date", keep="last").reset_index(drop=True)
    if work.empty:
        return work
    initial = float(baseline) if baseline is not None else float(work.iloc[0]["nav"])
...
def load_experiment(matrix_root: Path, name: str) -> dict[str, Any]:
    ...
    tracking_paths = experiment_tracking_paths(root)
    tracking_manifest = read_json(tracking_paths["manifest"], {}) or {}
    tracking_initial_cash = pd.to_numeric(pd.Series([tracking_manifest.get("initial_cash")]), errors="coerce").iloc[0]
    tracking_baseline = float(tracking_initial_cash) if pd.notna(tracking_initial_cash) else None
    tracking_nav = normalize_nav(read_csv(tracking_paths["nav"]), tracking_baseline)
    tracking_orders = _normalize_dates(read_csv(tracking_paths["orders"]))
    tracking_positions = _normalize_dates(read_csv(tracking_paths["positions"]))
    tracking_rejections = _normalize_dates(read_csv(tracking_paths["rejections"]))
    tracking_rebalance_dates = pd.DataFrame()
    if not tracking_nav.empty and "is_rebalance_day" in tracking_nav.columns:
        tracking_rebalance_dates = tracking_nav.loc[
            tracking_nav["is_rebalance_day"].astype(bool)
        ].copy()
    return {
        "identity": identity,
        "root": root,
        ...
        "tracking_manifest": tracking_manifest,
        "tracking_result": read_csv(tracking_paths["result"]),
        "tracking_nav": tracking_nav,
        "tracking_orders": tracking_orders,
        "tracking_positions": tracking_positions,
        "tracking_rejections": tracking_rejections,
        "tracking_rebalance_dates": tracking_rebalance_dates,
        "tracking_latest_state": read_json(tracking_paths["latest_state"], {}) or {},
        "tracking_latest_positions": read_csv(tracking_paths["latest_positions"]),
    }
...
def build_forward_comparison(matrix_root: Path, experiment_names: Iterable[str]) -> pd.DataFrame:
    ...
    for name in experiment_names:
        ...
        nav = item["tracking_nav"] if not item["tracking_nav"].empty else item["forward_nav"]
        if nav.empty:
            continue
        series.append(nav.set_index("date")["cumulative_return_pct"].rename(item["identity"].display_name))
