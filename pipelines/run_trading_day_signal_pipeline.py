...
def wait_proc(
    ...
    *,
    ...
    Returns True if the process finished normally.  Returns False when timeout
    is reached and the process is terminated.  This is critical for the 14:55
    signal path: slow AKShare collection must not block scoring indefinitely.
    """
    if dry_run or proc is None:
        return True
    start = time.time()
    log(f"[WAIT] {name} timeout={timeout_seconds}s ...")
    try:
        rc = proc.wait(timeout=timeout_seconds if timeout_seconds and timeout_seconds > 0 else None)
    except subprocess.TimeoutExpired:
        elapsed = time.time() - start
        log(f"[TIMEOUT] {name} still running after {elapsed:.1f}s")
...
def main() -> int:
    ...
    ap.add_argument("--spot-source-priority", default="sina_batch,ths_etf,xq", help="股票快照数据源优先级；14:55 默认使用按需小批量源，禁止全量 A 股/港股源。")
    ...
    ap.add_argument("--xq-per-symbol-timeout-seconds", type=float, default=2.0, help="单个 xq 补洞请求硬超时秒数。")
    ap.add_argument("--spot-source-timeout-seconds", type=float, default=5.0, help="单个实时数据源硬超时秒数；默认 5 秒。")
    ap.add_argument("--stock-collect-wait-timeout-seconds", type=float, default=45.0, help="build-time 后最多等待股票采集进程秒数；超时则终止并继续。")
    ...
    try:
        ...
        if not args.skip_stock_collect:
            stock_collect_cmd = [
                ...
                "--xq-per-symbol-timeout-seconds", str(args.xq_per_symbol_timeout_seconds),
                "--spot-source-timeout-seconds", str(args.spot_source_timeout_seconds),
            ]
        ...
        if stock_proc is not None:
            finished = wait_proc(
                ...
                timeout_seconds=args.stock_collect_wait_timeout_seconds,
            )
            mark("stock_collect_wait", "ok" if finished else "timeout_terminated")
        if context_proc is not None:
            finished = wait_proc(
                context_proc,
                "context_collect",
                dry_run=args.dry_run,
                timeout_seconds=args.context_collect_wait_timeout_seconds,
            )
            mark("context_collect_wait", "ok" if finished else "timeout_terminated")
