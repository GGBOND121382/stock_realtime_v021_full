...
# canonicalize_rebuild_and_save_models.sh
#
# FINAL SIMPLE FLOW:
#
#   1) Copy/merge existing:
#        saved_data/<code>_pipeline_out_v2_all14/
#      into canonical:
#        saved_data/<code>_pipeline_out/
#
#   2) Rebuild / complete required data in canonical folders:
#        saved_data/<code>_pipeline_out/
#
#   3) Train/save already-selected fixed model artifacts into:
#        saved_models/<stock_code>/<artifact_name>/
#
# No selected_good.
# No production run-tag folders.
# v2_all14 is kept by default. Delete it only after all pending failed3/v2_all14 tasks are finished.
#
# By default this script:
#   - syncs v2_all14 -> canonical with rsync --delete when rsync is available;
#   - deletes old v2_all14 source only after sync succeeds;
#   - rebuilds only necessary data for selected models;
#   - saves fixed selected artifacts.
#
# Usage:
#   chmod +x scripts/canonicalize_rebuild_and_save_models.sh
#   PYTHON=python3 END_DATE=2026-05-12 ./scripts/canonicalize_rebuild_and_save_models.sh
#
# Important options:
#   ONLY=600312.SH,601899.SH       only process selected symbols
#   SOURCE_SUFFIX=_v2_all14        source suffix to migrate from
#   DELETE_SOURCE=0                keep source after successful sync; default 0 because failed3 still uses v2_all14
#   SYNC_DELETE=1                  make canonical exactly match source first; default 1
#   SKIP_MIGRATE=1                 skip v2_all14 -> canonical migration
#   SKIP_REBUILD=1                 skip data rebuild
#   SKIP_SAVE=1                    skip model saving
#   FORCE_REBUILD=0                do not delete canonical before rebuild; default 0
#   CLEAN_MODEL_STOCK_DIR=1        clean saved_models/<stock_code>; default 1
#   ALLOW_MINIMAL_SEARCH_FALLBACK=1
#                                  only if run_nextday_pipeline.py has no data-only flag.
#                                  Default 0: refuse hidden search.
#   DRY_RUN=1                      print commands only
#
# NOTE:
#   If run_nextday_pipeline.py does not support --skip-search / --data-only / --no-search,
#   this script will stop unless ALLOW_MINIMAL_SEARCH_FALLBACK=1 is explicitly set.
...
ALLOW_MINIMAL_SEARCH_FALLBACK="${ALLOW_MINIMAL_SEARCH_FALLBACK:-0}"
DRY_RUN="${DRY_RUN:-0}"
ONLY="${ONLY:-}"
...
LOG_DIR="${LOG_DIR:-saved_data/model_search_queue_logs/canonicalize_rebuild_save_$(date +%Y%m%d_%H%M%S)}"
...
echo "symbol,sector,external,mode,status,returncode,start_time,end_time,log_file" > "$REBUILD_SUMMARY"
...
migrate_one() {
  ...
  if command -v rsync >/dev/null 2>&1; then
    ...
    # Use dot copy to include hidden files.
...
detect_data_only_flag() {
  local help_text
  help_text="$("$PYTHON" pipelines/run_nextday_pipeline.py --help 2>/dev/null || true)"
  if echo "$help_text" | grep -q -- "--skip-search"; then
    echo "--skip-search"
    return 0
  fi
  if echo "$help_text" | grep -q -- "--data-only"; then
    echo "--data-only"
    return 0
  fi
  if echo "$help_text" | grep -q -- "--no-search"; then
    echo "--no-search"
    return 0
  fi
  echo ""
...
DATA_ONLY_FLAG="$(detect_data_only_flag)"
...
build_data_cmd() {
  ...
  if [[ -n "$DATA_ONLY_FLAG" ]]; then
    cmd+=("$DATA_ONLY_FLAG")
  elif [[ "$ALLOW_MINIMAL_SEARCH_FALLBACK" == "1" ]]; then
    :
  else
    echo "[ERROR] run_nextday_pipeline.py has no data-only flag: --skip-search / --data-only / --no-search" >&2
    echo "[ERROR] Refusing to run any search by default." >&2
    echo "[ERROR] Add a data-only flag to pipeline, or rerun with ALLOW_MINIMAL_SEARCH_FALLBACK=1." >&2
    return 2
  fi
...
rebuild_data() {
  ...
  local mode_label
  if [[ -n "$DATA_ONLY_FLAG" ]]; then
    mode_label="data_only:${DATA_ONLY_FLAG}"
  else
    mode_label="minimal_selected_search_fallback"
  fi
  ...
  echo
  echo "============================================================"
  echo "[REBUILD DATA] $symbol sector=$sector external=$ext_label mode=$mode_label"
  ...
  mapfile -t cmd < <(build_data_cmd "$symbol" "$sector" "$external" "$groups" "$models" "$targets" "$entries")
  if [[ "${#cmd[@]}" -eq 0 ]]; then
    end_time="$(date '+%F %T')"
    echo "${symbol},${sector},${ext_label},${mode_label},failed_build_cmd,2,${start_time},${end_time},${log_file}" >> "$REBUILD_SUMMARY"
    return 2
  fi
  ...
  if [[ "$rc" -eq 0 ]]; then
    ...
    echo "${symbol},${sector},${ext_label},${mode_label},ok,${rc},${start_time},${end_time},${log_file}" >> "$REBUILD_SUMMARY"
    return 0
  else
    echo "[REBUILD FAIL] $symbol rc=$rc"
    echo "${symbol},${sector},${ext_label},${mode_label},failed,${rc},${start_time},${end_time},${log_file}" >> "$REBUILD_SUMMARY"
    return "$rc"
  fi
}
...
echo "data-only flag:${DATA_ONLY_FLAG:-<none>}"
echo "fallback:      $ALLOW_MINIMAL_SEARCH_FALLBACK"
echo "============================================================"

# Step 1: move/merge v2_all14 into canonical.
for s in "${SYMBOLS[@]}"; do
  migrate_one "$s" || MIGRATE_FAILED=$((MIGRATE_FAILED + 1))
done

# Step 2: rebuild / complete required data in canonical folders.
rebuild_data 600312.SH 电网设备 "" \
  "reversal_fundamental_regime,reversal_fundamental_regime_sector" \
  "xgb_d3_400_lr003_mcw3,xgb_d3_600_lr002_mcw3" \
  "close_profit" \
  "vwap_low,all_days" \
  || REBUILD_FAILED=$((REBUILD_FAILED + 1))

rebuild_data 601899.SH 贵金属 "" \
  "reversal_fundamental_regime_sector" \
  "extra_trees_600_d3,xgb_d3_400_lr003_mcw3" \
  "close_profit" \
  "vwap_low,all_days" \
  || REBUILD_FAILED=$((REBUILD_FAILED + 1))

rebuild_data 603308.SH 通用设备 "aero_nuclear_equipment" \
  "reversal_fundamental_regime_sector,reversal_fundamental_regime_sector_external" \
  "xgb_d3_400_lr003_mcw3,xgb_d3_600_lr002_mcw3,xgb_d4_500_lr002_mcw5" \
  "close_profit" \
  "vwap_low,all_days" \
  || REBUILD_FAILED=$((REBUILD_FAILED + 1))

rebuild_data 600096.SH 农化制品 "" \
  "reversal_fundamental_regime,reversal_fundamental_regime_sector" \
  "xgb_d3_400_lr003_mcw3,xgb_d4_500_lr002_mcw5" \
  "close_profit,hit80" \
  "vwap_low,all_days" \
  || REBUILD_FAILED=$((REBUILD_FAILED + 1))

rebuild_data 002311.SZ 农产品加工 "" \
  "reversal_fundamental_regime,reversal_fundamental_regime_sector" \
  "xgb_d3_600_lr002_mcw3,random_forest_600_d4,lgbm_leaves15_700" \
  "close_profit,hit80" \
  "vwap_low,all_days" \
  || REBUILD_FAILED=$((REBUILD_FAILED + 1))

rebuild_data 601985.SH 电力 "" \
...
# Step 3: save fixed selected models.
for s in "${SYMBOLS[@]}"; do
