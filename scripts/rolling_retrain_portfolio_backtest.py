...
def oof_validation_scores(
    ...
    train_rows_cfg = int(getattr(args, "train_rows", 0) or 0)
    for fold in folds:
        fold_start_idx = int(fold[0])
        fold_valid = history.loc[fold].copy()
        fold_train = history.loc[history.index < fold_start_idx].copy()
        if train_rows_cfg > 0:
            # Match the pipeline-style rolling window inside OOF validation:
            # each OOF fold may only train on the trailing train_rows before
            # the fold's validation slice, not on all earlier history.
            if len(fold_train) < train_rows_cfg:
                return None
            fold_train = fold_train.iloc[-train_rows_cfg:].copy()
...
def train_score_one(
    ...
    if cached is None:
        ...
        if len(history) <= valid_rows + 1:
            return None
        train_pool = history.iloc[:-valid_rows].copy()
        valid = history.iloc[-valid_rows:].copy()
        configured_train_rows = int(getattr(args, "train_rows", 0) or 0)
        if configured_train_rows > 0:
            # Pipeline-like mode: use only the trailing train_rows immediately
            # before the validation window. This avoids using very old regimes
            # while preserving the same trailing validation threshold logic.
            if len(train_pool) < configured_train_rows:
                return None
            train = train_pool.iloc[-configured_train_rows:].copy()
        else:
            # Legacy mode: use all history before the validation tail.
        ...
        if args.final_refit_with_valid or args.threshold_mode == "oof":
            if configured_train_rows > 0:
                # Optional production-style refit uses the same finite rolling
                # window plus the validation tail, not all history. Default is
                # still no final refit, matching the old threshold-model flow.
                final_window = history.iloc[-(configured_train_rows + valid_rows):].copy()
            else:
                final_window = history
            final_fit = final_window.loc[final_window["entry_signal"].to_numpy(bool)].copy()
        ...
        cached = {
            ...
            "configured_train_rows": int(configured_train_rows),
            "train_window_mode": "fixed_tail" if configured_train_rows > 0 else "all_history",
    ...
    return {
        ...
        "rolling_train_pool_rows": int(cached.get("train_pool_rows", cached["train_rows"])),
        "rolling_configured_train_rows": int(cached.get("configured_train_rows", 0)),
        "rolling_train_window_mode": str(cached.get("train_window_mode", "all_history")),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=0,
        help=(
            "If >0, use only the trailing N rows before the validation window "
            "for each rolling retrain. Example: --train-rows 756 makes the "
            "backtest closer to the pipeline search window train=756, valid=126. "
            "Default 0 preserves legacy all-history training."
        ),
    )
...
def main() -> int:
    ...
    summary.update({
        ...
        "train_window_mode": "fixed_tail" if int(args.train_rows or 0) > 0 else "all_history",
