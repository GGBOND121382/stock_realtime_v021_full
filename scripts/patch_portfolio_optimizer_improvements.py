...
'''
    txt = txt.replace(marker, inj, 1)

helper_marker = "def metric_bps_from_return"
helpers = r'''
...
def _rule_matches(row: Dict[str, Any], stock_code: str, artifact: str) -> bool:
    ...
    pat = str(
        row.get("artifact_pattern", row.get("artifact_name", row.get("model_name", "*"))) or "*"
    ).strip()
    if not pat or pat == "*":
        return True
    if pat == artifact:
...
def apply_override_fields(
    ...
    enabled = _truthy(ov.get("enabled", 1), True) and _truthy(
        rp.get("enabled", rp.get("recent_enabled", 1)), True
    )
...
'''
if helpers.strip() not in txt:
    txt = txt.replace(helper_marker, helpers + "\n" + helper_marker, 1)
...
txt = replace_once(txt, old_sig, new_sig, "build_inputs signature")

old_block = '''    raw = pd.read_csv(src)
    out_input_dir.mkdir(parents=True, exist_ok=True)

    signals_out = out_input_dir / "portfolio_signals.csv"
'''
new_block = '''    raw = pd.read_csv(src)
    out_input_dir.mkdir(parents=True, exist_ok=True)
    ...
    signals_out = out_input_dir / "portfolio_signals.csv"
'''
txt = replace_once(txt, old_block, new_block, "adapter context/override load")

old_sector = '''        sector = r.get("sector", r.get("industry", "UNKNOWN"))

        sig_rows.append({
'''
new_sector = '''        sector, sector_source = choose_sector(r, stock_code, context_sector_map)
        override_fields = apply_override_fields(stock_code, artifact, override_rows, recent_rows)

        sig_rows.append({
'''
txt = replace_once(txt, old_sector, new_sector, "sector override block")

old_sig_row_tail = '''            "metadata_path": str(meta_path) if meta_path else "",
        })
'''
new_sig_row_tail = '''            "metadata_path": str(meta_path) if meta_path else "",
            "sector_source": sector_source,
            **override_fields,
        })
'''
txt = replace_once(txt, old_sig_row_tail, new_sig_row_tail, "signal row override fields")

old_met_tail = '''            "sector": sector,
        })
'''
new_met_tail = '''            "sector": sector,
            "sector_source": sector_source,
            **override_fields,
        })
'''
txt = replace_once(txt, old_met_tail, new_met_tail, "metrics row override fields")

old_args = '''    ap.add_argument("--config", default="configs/portfolio_confirm_config.json")
    ap.add_argument("--out-dir", default="portfolio_reports")
'''
new_args = '''    ap.add_argument("--config", default="configs/portfolio_confirm_config.json")
    ...
    ap.add_argument("--out-dir", default="portfolio_reports")
'''
txt = replace_once(txt, old_args, new_args, "adapter args")

old_call = '''        use_all_scores=bool(args.use_all_scores),
    )
'''
new_call = '''        use_all_scores=bool(args.use_all_scores),
...
'''
txt = replace_once(txt, old_call, new_call, "adapter build_inputs call")

p.write_text(txt, encoding="utf-8")
print("[PATCHED]", p)

# ----------------------------------------------------------------------
# 2) scripts/run_portfolio_confirm_from_signals.sh
# ----------------------------------------------------------------------
p = Path("scripts/run_portfolio_confirm_from_signals.sh")
txt = p.read_text(encoding="utf-8")
old_vars = '''CONFIG="${CONFIG:-configs/portfolio_confirm_config.json}"
OUT_DIR="${OUT_DIR:-portfolio_reports}"
'''
new_vars = '''CONFIG="${CONFIG:-configs/portfolio_confirm_config.json}"
...
OUT_DIR="${OUT_DIR:-portfolio_reports}"
'''
txt = replace_once(txt, old_vars, new_vars, "portfolio wrapper vars")

old_cmd_cfg = '''if [[ -f "$HISTORY" ]]; then
  CMD+=(--history "$HISTORY")
else
...
CMD+=("${EXTRA_ARGS[@]}")
'''
txt = replace_once(txt, old_cmd_cfg, new_cmd_cfg, "portfolio wrapper extra adapter args")
p.write_text(txt, encoding="utf-8")
print("[PATCHED]", p)

# ----------------------------------------------------------------------
# 3) daily_portfolio_confirm_pyscipopt.py
# ----------------------------------------------------------------------
p = Path("portfolio_decision/daily_portfolio_confirm_pyscipopt.py")
txt = p.read_text(encoding="utf-8")

old_dataclass = '''    median_return_bps: float
    utility_bps: float
    current_value: float
'''
new_dataclass = '''    median_return_bps: float
    utility_bps: float
    enabled: bool
    ...
    current_value: float
'''
txt = replace_once(txt, old_dataclass, new_dataclass, "Candidate dataclass fields")

old_parse = '''def parse_drawdown_abs(x: Any, default: float = 0.12) -> float:
'''
bool_helper = '''def parse_bool_flag(x: Any, default: bool = True) -> bool:
    if x is None:
        return default
    try:
        if pd.isna(x):
...
if "def parse_bool_flag" not in txt:
    txt = txt.replace(old_parse, bool_helper + old_parse, 1)

old_after_price = '''        if not np.isfinite(price) or price <= 0:
...
'''
new_after_price = '''        if not np.isfinite(price) or price <= 0:
            rejected.append({"stock_code": code, "model_name": model_name, "reason": "missing_or_invalid_price"})
            continue
        ...
        override_reason = str(
            get_row_field(row, "model_override_reason", get_row_field(row, "recent_perf_note", "")) or ""
        )
        if not enabled:
        ...
        model_type = infer_model_type(label_mode, row, cfg, code)
'''
txt = replace_once(txt, old_after_price, new_after_price, "optimizer enabled/weight parse")

old_caps = '''        tier, max_weight, max_add_weight = get_tier_and_caps(row, code, model_type, cfg)
        tier_mult = as_float(cfg.get("tier_multiplier", {}).get(str(tier), 1.0), 1.0)

        vol_daily = float(vol_map.get(code, np.nan))
'''
new_caps = '''        tier, max_weight, max_add_weight = get_tier_and_caps(row, code, model_type, cfg)
        ...
        if np.isfinite(max_weight_override) and max_weight_override > 0:
            max_weight = float(max_weight_override)
        ...
        vol_daily = float(vol_map.get(code, np.nan))
'''
txt = replace_once(txt, old_caps, new_caps, "optimizer cap overrides")

old_utility = '''        utility_bps = (
...
new_utility = '''        utility_bps = (
        ...
        if utility_bps <= float(cfg.get("min_utility_bps", 0.0)):
'''
txt = replace_once(txt, old_utility, new_utility, "optimizer utility multiplier")

old_inst = '''            profit_factor=pf, trades=trades, median_return_bps=median_bps,
            utility_bps=utility_bps, current_value=current_value, current_shares=current_shares,
'''
new_inst = '''            profit_factor=pf, trades=trades, median_return_bps=median_bps,
...
'''
txt = replace_once(txt, old_inst, new_inst, "optimizer Candidate instantiation")

p.write_text(txt, encoding="utf-8")
print("[PATCHED]", p)

# ----------------------------------------------------------------------
# 4) historical backtest: model/stock perf summary, if script exists
# ----------------------------------------------------------------------
...
'''
    if "def write_perf_summaries" not in txt:
        ...
        if marker in txt:
            ...
            print("[WARN] cannot inject perf summary function; marker not found")

    old_summary = '''    summary = sim["summary"]
    summary.update({
'''
    new_summary = '''    perf_summary_paths = write_perf_summaries(sim["trades"], out_dir)
    summary = sim["summary"]
    summary.update({
        "perf_summary_paths": perf_summary_paths,
'''
    if old_summary in txt and "perf_summary_paths = write_perf_summaries" not in txt:
        txt = txt.replace(old_summary, new_summary, 1)

    p.write_text(txt, encoding="utf-8")
    print("[PATCHED]", p)

print("[OK] portfolio optimizer improvements patch applied")
