...
The CLI only selects the target, feature preset, fold, and training options.
Feature construction and fold semantics live in ``utils.as1455_ch17_common``.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="0=newest fold, 6=oldest fold",
    )
    ...
    parser.add_argument(
        "--retrain-best",
        action="store_true",
        help="Optional diagnostic retrain; backtests use search-time checkpoints",
    )
...
def main() -> None:
    ...
    if out_dir.exists() and any(out_dir.iterdir()) and not args.force:
        raise SystemExit(
            "output dir already has files; pass --force or choose another "
            f"--out-dir: {out_dir}"
        )
    ...
    train_idx, test_idx, fold = common.get_fold_target(
        features.X, args.fold_index, args.target_col
    )
    ...
    pd.DataFrame([fold]).to_csv(
        out_dir / "fold_report.csv", index=False, encoding="utf-8-sig"
    )
    common.write_json(
        out_dir / "rotation_feature_cols.json", features.rotation_cols
    )
    common.write_json(out_dir / "addon_feature_cols.json", features.addon_cols)
    common.write_json(
        out_dir / "feature_group_cols.json", features.feature_groups
    )
    common.write_json(out_dir / "feature_cols_final.json", list(features.X.columns))
    common.write_json(
        out_dir / "sector_onehot_cols.json", features.sector_onehot_cols
    )
    pd.DataFrame(grid).to_csv(
        out_dir / "param_grid.csv", index=False, encoding="utf-8-sig"
    )
