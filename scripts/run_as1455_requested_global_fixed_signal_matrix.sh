...
TARGET_FOLDS_R05="${TARGET_FOLDS_R05:-0,1,2,3,4,5}"
TARGET_FOLDS_R01="${TARGET_FOLDS_R01:-0,1,2,3,4,5}"
TARGET_FOLDS_R21="${TARGET_FOLDS_R21:-0,1,2,3,4}"
...
"$PYTHON_BIN" -m py_compile \
  ...
  scripts/check_as1455_requested_fixed_signal_matrix.py
...
prepare_target_cache() {
  ...
  local nested_root="${4:-}"
  echo "===== prepare cache: target=$target_col rebalance=$rebalance_every folds=$target_folds ====="
  local args=(
    PYTHON_BIN="$PYTHON_BIN"
...
prepare_target_cache \
  r05_fwd 5 "$TARGET_FOLDS_R05" \
  "saved_data/ashare_ml4t/ch17_as1455_nested_fold_protocol/r05_addon_nested_v1"
run_experiment \
  r05_fwd 5 "$TARGET_FOLDS_R05" all5 \
  r05_all5_reb5_fold0_5_forward

prepare_target_cache r01_fwd 1 "$TARGET_FOLDS_R01"
run_experiment \
  r01_fwd 1 "$TARGET_FOLDS_R01" all5 \
  r01_all5_reb1_fold0_5_forward
run_experiment \
  r01_fwd 1 "$TARGET_FOLDS_R01" first3 \
  r01_first3_reb1_fold0_5_forward
run_experiment \
  r01_fwd 1 "$TARGET_FOLDS_R01" best \
  r01_best_reb1_fold0_5_forward

# r21 source_fold6 is unavailable because the sample is too short.  Use the
# complete available one-fold-lag development set: source_fold5..1 -> target_fold4..0.
prepare_target_cache r21_fwd 21 "$TARGET_FOLDS_R21"
run_experiment \
  r21_fwd 21 "$TARGET_FOLDS_R21" all5 \
  r21_all5_reb21_fold0_4_forward
run_experiment \
  r21_fwd 21 "$TARGET_FOLDS_R21" first3 \
  r21_first3_reb21_fold0_4_forward
run_experiment \
  r21_fwd 21 "$TARGET_FOLDS_R21" best \
  r21_best_reb21_fold0_4_forward
...
summary.to_csv(
    root / "fixed_signal_matrix_summary.csv",
    index=False,
    encoding="utf-8-sig",
)
    ...
    json.dumps(
        {
            ...
            "r05_target_folds": [0, 1, 2, 3, 4, 5],
            "r01_target_folds": [0, 1, 2, 3, 4, 5],
            "r21_target_folds": [0, 1, 2, 3, 4],
            "r21_target_fold5_skipped_reason": (
                "source_fold6 unavailable because r21 sample length is insufficient"
            ),
