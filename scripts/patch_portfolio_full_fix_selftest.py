...
def find_func(text: str, name: str, next_name: str) -> tuple[int, int]:
    start_marker = f"\ndef {name}("
    next_marker = f"\ndef {next_name}("
...
def ensure_usage_dataclass_fields(text: str, class_name: str, anchor: str) -> str:
    ...
    usage = {"entry_policy", "label_mode", "expected_return_col", "samples", "metadata_path"}
    lines = []
    ...
    for line in lines:
        ...
        if line.strip().startswith(anchor + ":"):
            ...
            inserted = True
    if not inserted:
        raise RuntimeError(f"anchor {anchor} not found in {class_name}")
...
def patch_adapter() -> None:
    ...
    start, end = find_func(text, "build_inputs", "make_account_template")
    text = text[:start] + BUILD_INPUTS.strip() + "\n\n" + text[end:]
    write(ADAPTER, text)
...
def patch_backtest() -> None:
    ...
    if "from model_training.optimize_nextday_vwap_model import add_trade_returns" not in text:
        old = """if str(PROJECT_DIR) not in sys.path:
    sys.path.insert(0, str(PROJECT_DIR))


DEFAULT_EVAL_CONFIG = {
"""
        new = """if str(PROJECT_DIR) not in sys.path:
    sys.path.insert(0, str(PROJECT_DIR))
...
DEFAULT_EVAL_CONFIG = {
"""
        text = replace_once(text, old, new, "backtest import add_trade_returns")

    if "training_samples*.csv may not persist these columns" not in text:
        old = """        samples = samples.dropna(subset=["date"]).sort_values("date")

        cols = read_feature_columns(artifact_dir)
"""
        new = """        samples = samples.dropna(subset=["date"]).sort_values("date")
        ...
        cols = read_feature_columns(artifact_dir)
"""
        text = replace_once(text, old, new, "backtest add_trade_returns")

    text = ensure_usage_dataclass_fields(text, "OpenLot", "utility_bps")
    text = ensure_usage_dataclass_fields(text, "TradeRecord", "exit_reason")

    if 'expected_return_col=str(r.get("expected_return_col"' not in text:
        old = """                    utility_bps=as_float(r.get("utility_bps", np.nan), np.nan),
                ))
"""
        new = """                    utility_bps=as_float(r.get("utility_bps", np.nan), np.nan),
...
"""
        text = replace_once(text, old, new, "OpenLot propagated fields")

    if 'exit_reason="scheduled_hold_days",\n                    entry_policy=lot.entry_policy' not in text:
        old = """                    utility_bps=lot.utility_bps,
                    exit_reason="scheduled_hold_days",
"""
        new = """                    utility_bps=lot.utility_bps,
                    exit_reason="scheduled_hold_days",
...
"""
        text = replace_once(text, old, new, "scheduled TradeRecord fields")
...
"""
        new = """                utility_bps=lot.utility_bps,
                ...
                metadata_path=lot.metadata_path,
"""
        text = replace_once(text, old, new, "force-close TradeRecord fields")
...
"""
        text = replace_once(text, old, new, "rejected mask")
...
"""
    ...
    ev_bps: float
"""
        text = replace_once(text, old, new, "Candidate fields")
...
"""
        ...
        pred_prob = parse_rate_decimal(get_row_field(row, "pred_prob", get_row_field(row, "win_rate", cfg["default_hit_prob"])), cfg["default_hit_prob"])
"""
        text = replace_once(text, old, new, "Candidate extracted fields")
...
"""
        new = """            target_hit_bps=target_hit_bps, fail_loss_bps=fail_loss_bps,
            ...
            quality_weight=quality, tier=tier, tier_multiplier=tier_mult, vol_daily=vol_daily,
"""
        text = replace_once(text, old, new, "Candidate construction fields")
        ...
        for key in ["signals", "metrics"]:
            ...
            for col in ["entry_policy", "entry_vwap_premium_bps", "samples", "expected_return_col"]:
                if col not in df.columns:
                    raise RuntimeError(f"synthetic {key} missing column {col}")
    ...
    for cls_name in ["OpenLot", "TradeRecord"]:
        ...
        for f in fields(cls):
            ...
            if has_default:
                ...
                bad.append(f.name)
        if bad:
            raise RuntimeError(f"{cls_name} dataclass default order invalid: {bad}")
...
def main() -> int:
    ...
    backup_root = Path("saved_data/patch_backups/portfolio_full_fix_selftest")
    ...
    try:
        ...
        patch_optimizer()
        validate_all()
        log("[OK] full portfolio fix applied and self-tested")
        return 0
